...
function parseArgs() {
  ...
  const result = {
    ...
    init: false,   // --init (v1.0)
    verify: false, // --verify (v1.0)
    inputFile: null,
    replyTo: null,
    note: null,
    query: null
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    else if (args[i] === "--init") { result.init = true; consumedIdx.add(i); }
    else if (args[i] === "--verify") { result.verify = true; consumedIdx.add(i); }
...
const isUtilMode = ARGS.init || ARGS.verify;

if (!isReadMode && !isUtilMode && !ARGS.query) {
  console.error(`XRPL Echo Breaker v1.0 — エコーチェンバーを破壊する多角的記録プロトコル

--- はじめに ---
  node echo-breaker.js --init                                       # 初回セットアップ
...
--- 検証する ---
  node echo-breaker.js --verify                                     # ハッシュ連鎖を全検証`);
  process.exit(1);
}
...
async function cmdInit() {
  ...
  console.log(`
╔═══════════════════════════════════════════════════════╗
║  XRPL Echo Breaker v1.0 — セットアップ               ║
╚═══════════════════════════════════════════════════════╝
`);
  console.log("このツールは多角的視点をXRP Ledger テストネットに永久記録します。");
  console.log("報酬ゼロ・純粋利他的。真理を決めない。視点を残す。\n");

  // 1. records ディレクトリ
  if (!fs.existsSync(RECORDS_DIR)) {
    fs.mkdirSync(RECORDS_DIR);
    console.log("✅ records/ ディレクトリを作成しました");
  } else {
    const count = fs.readdirSync(RECORDS_DIR).filter(f => RECORD_PATTERN.test(f)).length;
    console.log(`✅ records/ ディレクトリ確認済み（${count}件の記録あり）`);
  }

  // 2. .env 設定
  const envPath = path.join(__dirname, '.env');
  if (fs.existsSync(envPath)) {
    console.log("✅ .env ファイル確認済み");
  } else {
    console.log("\n📝 Grok API キーの設定（オプション）");
    console.log("   キーがなくてもフォールバックエンジンで動作します。");
    console.log("   BYOAIモード（--input）なら外部AIの視点をそのまま記録できます。\n");
    const key = await ask("   Grok API Key (スキップは Enter): ");
    if (key.trim()) {
      fs.writeFileSync(envPath, `GROK_API_KEY=${key.trim()}\n`);
      console.log("   ✅ .env ファイルを作成しました");
    } else {
      console.log("   ⏭️  スキップ（フォールバック + BYOAIモードで動作）");
    }
  }
  ...
  try {
    ...
    console.log("   ✅ テストネット接続OK");
    await client.disconnect();
  } catch (err) {
    console.log(`   ⚠️  接続失敗: ${err.message}`);
    console.log("   ネットワークを確認してください。オフラインでも --dry-run は使えます。");
  }
...
// ==================== --verify ハッシュ連鎖検証 (v1.0) ====================
function cmdVerify() {
  if (!fs.existsSync(RECORDS_DIR)) {
    console.log("❌ records/ ディレクトリがありません。--init で初期化してください。");
    return;
  }
  ...
  console.log(`\n🔗 ハッシュ連鎖検証 (${files.length}件)\n`);

  let expectedPrevHash = GENESIS.txHash;
  let ok = 0;
  let broken = 0;
  ...
  files.forEach((file, idx) => {
    ...
    // prev_hash チェック
    const prevOk = data.prev_hash === expectedPrevHash;

    // 記録内のハッシュ整合性（改ざんチェック）
    const num = data.record_number || "???";

    if (prevOk) {
      console.log(`  ✅ #${num}  prev_hash OK  hash: ${actualHash.slice(0, 16)}...`);
      ok++;
    } else {
      console.log(`  ❌ #${num}  prev_hash MISMATCH`);
      console.log(`     期待: ${expectedPrevHash.slice(0, 32)}...`);
      console.log(`     実際: ${(data.prev_hash || "").slice(0, 32)}...`);
      broken++;
    }
  ...
  console.log(`\n━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━`);
  console.log(`  Genesis Tx: ${GENESIS.txHash.slice(0, 24)}...`);
  console.log(`  検証結果 : ${ok}件 OK / ${broken}件 不整合`);

  if (broken === 0) {
    console.log(`  ✅ 全記録の連鎖が正常です。改ざんは検出されませんでした。`);
  } else {
    console.log(`  ⚠️  ${broken}件のハッシュ不整合を検出。記録が改ざんされた可能性があります。`);
  }
  console.log(`━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\n`);
}
...
// ==================== メイン ====================
async function main() {
  // ユーティリティモード (v1.0) — init / verify
  if (ARGS.init)              { await cmdInit();        return; }
  if (ARGS.verify)            { cmdVerify();            return; }
  ...
  console.log("🚀 XRPL Echo Breaker v1.0 起動...\n");
  ...
  if (ARGS.compare) {
    ...
    fullRecord = {
      ...
      prev_hash: prevHash,
      meta: { engine: "multi-engine-comparison", version: "v1.0" }
    };
