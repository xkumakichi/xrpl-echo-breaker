...
if (!ARGS.query) {
  ...
  node echo-breaker.js --compare "クエリ"                          # 全エンジンで同時検証 (v0.8)
  node echo-breaker.js --compare --input views.json "クエリ"       # BYOAI込みで比較 (v0.8)`);
  process.exit(1);
}
...
// ==================== メイン ====================
async function main() {
  console.log("🚀 XRPL Echo Breaker v0.8 起動...\n");
  ...
  if (ARGS.compare) {
    ...
    fullRecord = {
      ...
      prev_hash: prevHash,
      meta: { engine: "multi-engine-comparison", version: "v0.8" }
    };
